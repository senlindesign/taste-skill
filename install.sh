...
for a in "$@"; do
  case "$a" in
    ...
    -h|--help)
      ...
      echo "  With no tool flag, auto-detects installed CLIs. A tool flag forces that target."
      exit 0 ;;
...
# Auto-detect only when no explicit target is given; an explicit --tool flag forces it.
enabled(){ [ -n "$ONLY" ] || command -v "$1" >/dev/null 2>&1 || [ -d "$2" ]; }
...
any=0
if want claude && enabled claude "$CLAUDE_HOME"; then
...
if want codex && enabled codex "$CODEX_HOME"; then
  any=1; echo "== Codex CLI =="; link "$CODEX_HOME/skills"
  [ "$UNINSTALL" = 1 ] || printf '   Playwright MCP — add to ~/.codex/config.toml (server name must be "playwright"):\n       [mcp_servers.playwright]\n       command = "npx"\n       args = ["-y", "@playwright/mcp@latest", "--isolated"]\n'
fi
if want gemini && enabled gemini "$GEMINI_HOME"; then
  any=1; echo "== Gemini CLI =="; link "$GEMINI_HOME/skills"
  [ "$UNINSTALL" = 1 ] || printf '   Playwright MCP — add to ~/.gemini/settings.json (server name must be "playwright"):\n       { "mcpServers": { "playwright": { "command": "npx", "args": ["-y", "@playwright/mcp@latest", "--isolated"] } } }\n'
fi
...
if [ "$UNINSTALL" = 1 ]; then
  ...
  echo "Update later: git pull here (symlinks pick it up) — or for the Claude plugin install, /plugin update taste."
fi
